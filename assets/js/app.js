...
const APP_VERSION = 'ALARMAS-LUCIA-v3.1-AHORRO-IPHONE-SIN-BLOQUEO-20260718';
...
const state = {
  ...
  screenWakeLockHealthTimer: null,
  screenAwakeFallbackActive: false,
  screenAwakeFallbackLastError: '',
...
function hasEnergySaverWakeReason(){
  if(!state.energySaverActive) return false;
  if(state.energySaverAlarmId){
    const alarm = state.alarms.find(item => item && item.id === state.energySaverAlarmId);
    if(alarm && alarm.direction === 'outgoing' && (!alarm.from || alarm.from === state.deviceId) && isActiveAlarm(alarm)) return true;
  }
  return hasOwnActiveAlarm();
}

function shouldKeepScreenAwake(){
  if(isNativeAndroidLocationAvailable()) return false;
  // El modo AHORRO DE ENERGÍA necesita el bloqueo de pantalla aunque el emisor
  // no haya autorizado GPS. Antes solo se mantenía si había GPS activo, por lo
  // que una sincronización posterior podía liberar el bloqueo en el iPhone.
  return hasEnergySaverWakeReason() || hasOwnActiveGpsAlarm();
}

function getScreenAwakeFallbackVideo(){
  const video = $('screenAwakeFallbackVideo');
  if(!video) return null;
  if(video.dataset.noSleepConfigured === '1') return video;

  video.dataset.noSleepConfigured = '1';
  ...
  video.addEventListener('loadedmetadata', () => {
    // El recurso WebM dura menos de un segundo y puede repetirse con loop.
    // Safari usa el MP4 más largo: se mantiene dentro del primer medio segundo,
    // siguiendo el mecanismo probado por NoSleep.js para iOS antiguos.
    if(Number(video.duration || 0) <= 1){
      video.loop = true;
      return;
    }
    video.loop = false;
  ...
  video.addEventListener('timeupdate', () => {
    if(Number(video.duration || 0) > 1 && video.currentTime > 0.5){
      try { video.currentTime = Math.random() * 0.2; } catch {}
    }
  });

  video.addEventListener('ended', () => {
    if(shouldKeepScreenAwake() && document.visibilityState === 'visible'){
      enableScreenAwakeFallback(false);
    }
  });
...
async function enableScreenAwakeFallback(force = false){
  if(isNativeAndroidLocationAvailable()) return false;
  if(!force && !shouldKeepScreenAwake()) return false;
  if(document.visibilityState !== 'visible') return false;
  if(!isAppleMobileWebApp()) return false;

  const video = getScreenAwakeFallbackVideo();
  if(!video) return false;
  if(!video.paused && !video.ended){
    state.screenAwakeFallbackActive = true;
    return true;
  }

  try {
    // play() se invoca directamente dentro de la pulsación de ENVIAR o AHORRO
    // cuando force=true. Esto es imprescindible para las restricciones de iOS.
    const playPromise = video.play();
    if(playPromise && typeof playPromise.then === 'function') await playPromise;
    state.screenAwakeFallbackActive = !video.paused;
    state.screenAwakeFallbackLastError = '';
    return state.screenAwakeFallbackActive;
  } catch(err){
    state.screenAwakeFallbackActive = false;
    state.screenAwakeFallbackLastError = String(err?.name || err?.message || 'video-fallback-error');
    console.warn('No se pudo activar el respaldo anti-bloqueo de iPhone:', err);
    return false;
  }
}

function disableScreenAwakeFallback(){
  const video = getScreenAwakeFallbackVideo();
  state.screenAwakeFallbackActive = false;
  if(!video) return;
...
function clearScreenWakeLockRetry(){
  if(state.screenWakeLockRetryTimer){
    clearTimeout(state.screenWakeLockRetryTimer);
    state.screenWakeLockRetryTimer = null;
  }
}

function scheduleScreenWakeLockRetry(delayMs = 350){
  if(!shouldKeepScreenAwake() || document.visibilityState !== 'visible') return;
  clearScreenWakeLockRetry();
  state.screenWakeLockRetryTimer = setTimeout(() => {
    state.screenWakeLockRetryTimer = null;
    if(shouldKeepScreenAwake() && document.visibilityState === 'visible') requestScreenWakeLock(false);
...
async function requestScreenWakeLock(force = false){
  ...
  // En iPhone se activa también un vídeo local silencioso como respaldo. La
  // llamada comienza antes de cualquier await para conservar el gesto del usuario.
  const fallbackPromise = isAppleMobileWebApp()
    ? enableScreenAwakeFallback(force)
    : Promise.resolve(false);

  if(!navigator.wakeLock || typeof navigator.wakeLock.request !== 'function'){
    state.screenWakeLockLastError = 'unsupported';
    return Boolean(await fallbackPromise);
  }
  if(state.screenWakeLock && state.screenWakeLock.released === false){
    await fallbackPromise;
    return true;
  }
  if(state.screenWakeLockRequestPending) return Boolean(await fallbackPromise);
  ...
  try {
    ...
    sentinel.addEventListener('release', () => {
      ...
      // Safari puede liberar el bloqueo por un cambio de estado del sistema.
      // Si la alarma sigue activa, se recupera sin esperar otra pulsación.
      if(shouldKeepScreenAwake()) scheduleScreenWakeLockRetry(250);
    });
    await fallbackPromise;
    return true;
  } catch(err){
    ...
    const fallbackActive = Boolean(await fallbackPromise);
    if(shouldKeepScreenAwake()) scheduleScreenWakeLockRetry(1500);
...
function verifyScreenAwakeProtection(){
  if(document.visibilityState !== 'visible') return;
  if(!shouldKeepScreenAwake()){
    if(state.screenWakeLock || state.screenAwakeFallbackActive) releaseScreenWakeLock();
    return;
  }

  const nativeLockMissing = !state.screenWakeLock || state.screenWakeLock.released !== false;
  const fallbackVideo = getScreenAwakeFallbackVideo();
  const fallbackMissing = isAppleMobileWebApp() && (!fallbackVideo || fallbackVideo.paused || fallbackVideo.ended);
  if(nativeLockMissing && !state.screenWakeLockRequestPending) requestScreenWakeLock(false);
  else if(fallbackMissing) enableScreenAwakeFallback(false);
}
...
function setupScreenWakeLockLifecycle(){
  ...
  const resumeWebTracking = () => {
    ...
    setTimeout(verifyScreenAwakeProtection, 300);
  };
  ...
  // En iPhone, cualquier interacción permite reactivar tanto Wake Lock como el
  // respaldo de vídeo si Safari los suspendió al volver desde otra aplicación.
  ...
  if(state.screenWakeLockHealthTimer) clearInterval(state.screenWakeLockHealthTimer);
  state.screenWakeLockHealthTimer = setInterval(verifyScreenAwakeProtection, 5000);
}
...
function enterEnergySaverMode(alarmId){
  ...
  // Se activan simultáneamente Wake Lock y el respaldo multimedia de iOS.
  // Al estar dentro del clic del usuario, Safari permite iniciar ambos.
