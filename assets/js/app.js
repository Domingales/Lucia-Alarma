...
const APP_VERSION = 'ALARMAS-LUCIA-v2.2-GPS-SEGUNDO-PLANO-DISPOSITIVOS-20260712';
...
function nativeStartImmobilityMonitor(alarm){
  ...
  const configKey = `${getImmobilityDurationMs()}|${getImmobilityRadiusM()}|${pollIntervalMs}|${state.pairKey || alarm.pairKey || ''}`;
  try {
...
async function sendAlarm(){
  ...
  try {
    ...
    const gpsOkText = isNativeAndroidLocationAvailable()
      ? `Alarma enviada online. GPS nativo activo: seguirá enviando una ubicación ${intervalText} con la app minimizada o la pantalla apagada.`
      : `Alarma enviada online. GPS activo: se enviará una ubicación ${intervalText} mientras el navegador mantenga la página activa.`;
...
function resumeSenderGpsForOwnActiveAlarms(){
  state.alarms.filter(a => a.from === state.deviceId && a.gpsEnabledBySender && isActiveAlarm(a)).forEach(a => {
    if(state.gpsPermission === 'granted' || nativeHasLocationPermission()) startSenderGps(a);
  });
}
...
function evaluateAllImmobilityStates(){
  state.alarms.filter(a => a.direction === 'incoming' && isActiveAlarm(a)).forEach(a => evaluateImmobilityForAlarm(a.id));
}
function evaluateImmobilityForAlarm(alarmId){
  ...
  const tracker = state.webImmobilityState[alarmId] || { active: false, anchorTimestamp: 0 };
  if(maxDistance <= radiusM){
    if(!tracker.active){
      tracker.active = true;
      tracker.anchorTimestamp = Number(anchor.timestamp);
      tracker.alertedAt = Date.now();
...
function showImmobilityWarning(alarm, durationMs, radiusM, maxDistance, latestLog){
  const minutes = Math.max(1, Math.round(durationMs / 60000));
  const distance = Math.max(0, Math.round(maxDistance));
  openAppDialog({
    title: 'Posible falta de movimiento',
    message: `El emisor de “${alarm.message || 'esta alarma'}” lleva al menos ${minutes} ${minutes === 1 ? 'minuto' : 'minutos'} dentro de una zona de ${radiusM} m. La separación máxima observada ha sido de ${distance} m. Último registro: ${formatDate(latestLog.timestamp)} ${formatTimeWithSeconds(latestLog.timestamp)}.`,
...
function maybeShowRemoteImmobilityAlert(alarm){
  ...
  const alertAt = Number(alarm.immobilityAlertAt || 0);
  if(!alertAt || state.immobilityShown[alarm.id] === alertAt) return;
  state.immobilityShown[alarm.id] = alertAt;
  saveImmobilityShown();
