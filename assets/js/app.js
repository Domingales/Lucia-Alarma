...
const APP_VERSION = 'ALARMAS-LUCIA-v2.9-MODO-3D-INTENSO-20260715';
...
function setupEvents(){
  ...
  ['vibrationEnabledInput', 'generalNotificationSoundInput', 'newAlarmSoundInput', 'cancelledAlarmSoundInput', 'scheduledAlarmSoundInput']
    .forEach(id => on(id, 'change', handleAlertSettingChange));
  on('testSoundBtn', 'click', () => { startTone(true); setTimeout(stopTone, 1800); });
...
function applySettings(){
  ...
  const vibration = $('vibrationEnabledInput'); if(vibration) vibration.checked = isVibrationEnabled();
  const generalNotificationSound = $('generalNotificationSoundInput'); if(generalNotificationSound) generalNotificationSound.checked = isGeneralNotificationSoundEnabled();
  const newAlarmSound = $('newAlarmSoundInput'); if(newAlarmSound) newAlarmSound.checked = isNewAlarmSoundEnabled();
  const cancelledAlarmSound = $('cancelledAlarmSoundInput'); if(cancelledAlarmSound) cancelledAlarmSound.checked = isCancelledAlarmSoundEnabled();
  const scheduledAlarmSound = $('scheduledAlarmSoundInput'); if(scheduledAlarmSound) scheduledAlarmSound.checked = isScheduledAlarmSoundEnabled();
  applyGpsSettingsUI();
...
function isVibrationEnabled(){ return state.settings.vibrationEnabled !== false; }
function isGeneralNotificationSoundEnabled(){ return state.settings.generalNotificationSoundEnabled !== false; }
function isNewAlarmSoundEnabled(){ return state.settings.newAlarmSoundEnabled !== false; }
function isCancelledAlarmSoundEnabled(){ return state.settings.cancelledAlarmSoundEnabled !== false; }
function isScheduledAlarmSoundEnabled(){ return state.settings.scheduledAlarmSoundEnabled !== false; }
...
function handleAlertSettingChange(){
  ...
  saveSettings();
  syncNativeAlertSettings();
  if(!isGeneralNotificationSoundEnabled()) stopImmobilityTone();
  if(!isScheduledAlarmSoundEnabled()) stopTone();
  else if(state.activeRingingAlarmId) startTone();
}
...
function syncNativeAlertSettings(){
  if(!hasAndroidBridgeMethod('saveAlertSettings')) return false;
  try {
    AndroidBridge.saveAlertSettings(
      Boolean(isVibrationEnabled()),
      Boolean(isGeneralNotificationSoundEnabled()),
      Boolean(isNewAlarmSoundEnabled()),
      Boolean(isCancelledAlarmSoundEnabled()),
      Boolean(isScheduledAlarmSoundEnabled())
    );
    return true;
  } catch(err){
    console.warn('No se pudieron guardar los ajustes nativos de avisos:', err);
    return false;
  }
}
...
async function showImmobilityWarning(alarm, durationMs, radiusM, maxDistance, latestLog, options={}){
  ...
  if(stage === 'initial'){
    if(isGeneralNotificationSoundEnabled() && !options.nativeSoundPlayed) playImmobilityToneOnce();
    await openAppDialog({
  ...
  if(isGeneralNotificationSoundEnabled() && !options.nativeSoundActive) startImmobilityTone(alarm.id);
...
function gpsLogHtml(log){
  ...
  return `<div class="gps-line">${escapeHtml(formatDate(log.timestamp))} · ${escapeHtml(formatTimeWithSeconds(log.timestamp))}<br>${coordinates} · ±${acc} m</div>`;
}
...
async function showRingingAlarm(alarm){
  ...
  if(isScheduledAlarmSoundEnabled()) startTone();
...
function startTone(force=false){
  if(!force && !isScheduledAlarmSoundEnabled()) return;
  stopTone();
  try {
...
function playImmobilityToneOnce(){
  if(!isGeneralNotificationSoundEnabled()) return;
  try {
    const AudioContext = window.AudioContext || window.webkitAudioContext;
    state.audioCtx = state.audioCtx || new AudioContext();
